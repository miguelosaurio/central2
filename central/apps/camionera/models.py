...
class camion(models.Model):
	...
	def __unicode__(self):
		return '%s %s' %(self.capacidad, self.clase)
...
class viaje(models.Model):
	...
	def __unicode__(self):
		return '%s %s %s %s %s' %(self.ruta, self.camion,self.chofer1,self.fechaSalida, self.contados)
...
class reservacion(models.Model):
	...
	def __unicode__(self):
		return '%s %s %s %s %s' %(self.viaje, self.asiento,self.nombre,self.apellidos, self.status)
...
class abordar(models.Model):
	...
	def __unicode__(self):
		return '%s %s %s' %(self.reservacion, self.status,self.fechaAbordar)
